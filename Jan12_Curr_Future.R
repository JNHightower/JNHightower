...
setwd('C:/Program Files/R/R2121/R2121/library/BIOMOD/BIOMOD/R/Jan12_Curr_Future/')
dir()

Coord<-read.csv("CoorXY_Current.csv", header=TRUE)
CoorXY<-Coord[,2:3]
head(CoorXY)
nrow(CoorXY)
...
gold<-read.csv("GW_BW_Current_9_variables.csv", header=TRUE)
length(gold)
nrow(gold)
head(gold)
names(gold)
...
multiple.plot(gold[,8:15], CoorXY[,1:2], cex=0.5)
#########################################################################################
Initial.State(sp.name="GW",Response = gold[,6], Explanatory = gold[,8:15],
IndependentResponse = gold[,6], IndependentExplanatory = gold[,8:15])
##########################################################################################
ls()
head(DataBIOMOD)
Biomod.material
...
MARS = T, RF = T, NbRunEval = 2, DataSplit = 70, Yweights=NULL, Roc = T, 
Optimized.Threshold.Roc = T,Kappa = T, TSS=T, KeepPredIndependent = T, 
VarImport=5, NbRepPA=1, strategy="circles",coor=CoorXY,distance=0.5,nb.absences=500)
...
@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
load("pred/Pred_GW")
GW_GAM=cbind(CoorXY[Biomod.PA.sample[[1]][[1]],],Pred_GW[,"GAM",1,1])
...
load("pred/Pred_GW")
GW_RF=cbind(CoorXY[Biomod.PA.sample[[1]][[1]],],Pred_GW[,"RF",1,1])
...
load("pred/Pred_GW_BinRoc")
Pred_GW_BinRoc[,3,,]
GWWA_mod_binary=cbind(Pred_GWWA_BinRoc, CoorXY)
...
A2a<-read.csv("GW_BW_9_variables_A2a.csv", header=TRUE)
names(A2a)
attach(A2a)
coordinates<-read.csv("CoorXY_A2a.csv", header=TRUE)

Projection(Proj = A2a[,8:15], Proj.name='A2a', GLM = T, GBM = T, GAM = T,
